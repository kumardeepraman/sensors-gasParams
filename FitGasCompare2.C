    ...
    const char* outPNG)
{
    ...
    double deltaYmin[3] = {-0.9, -0.19, -0.12};
...
void FitGasCompare2(){
    ...
    bool ifGas = false; //set it false for Aerogel
    ...
    for (int sipm = 0; sipm < nSiPM; sipm++) {
        ...
        int inRad = 1; int fiRad = 2;
        if(ifGas){inRad = 0; fiRad =1;}

        for (int irad = inRad; irad < fiRad; irad++) {
